...
module.exports = {
   ...
   placeOrder: (order, products, total) => {
      return new Promise((resolve, reject) => {
         ...
         let status =
            order["payment-method"] === "COD" ? "Order Placed" : "pending";
         let orderObj = {
            deliveryDetails: {
               name: order.name,
   ...
   placeOrderPaypal: (order, products, total) => {
      return new Promise((resolve, reject) => {
         //   console.log(order, products, total);
         let status =
            order["payment-method"] === "PAYPAL" ? "pending" : "reject";
         let orderObj = {
            deliveryDetails: {
               name: order.name,
               address: order.address,
   ...
   cancelledOrders: (userId) => {
      return new Promise(async (resolve, reject) => {
         ...
         let cancelledOrders = await db
            .get()
            .collection(collection.CANCELLED_ORDER_COLLECTIONS)
            .find({ userId: objectId(userId) })
            .toArray();
   ...
   cancelOrders: (orderId) => {
      return new Promise(async (resolve, reject) => {
         let cancelledOrder = await db
            .get()
            .collection(collection.ORDER_COLLECTIONS)
            .findOne({ _id: objectId(orderId) });
         db.get()
            .collection(collection.CANCELLED_ORDER_COLLECTIONS)
            .insertOne(cancelledOrder);
   ...
   getCancelledProducts: (orderId) => {
      return new Promise(async (resolve, reject) => {
         let cancelledOrderItems = await db
            .get()
            .collection(collection.CANCELLED_ORDER_COLLECTIONS)
            .aggregate([
               {
...
//         console.log("total", total[0].total);
   ...
   getMonthReport: (lastMonth) => {
      return new Promise(async (resolve, reject) => {
         let lastMonthReport = await db
            .get()
            .collection(collection.ORDER_COLLECTIONS)
            .find({ date: { $gte: lastMonth } })
            .toArray();
...
//         console.log("total", total[0].total);
   ...
   getYearReport: (lastYear) => {
      return new Promise(async (resolve, reject) => {
         let lastYearReport = await db
            .get()
            .collection(collection.ORDER_COLLECTIONS)
            .find({ date: { $gte: lastYear } })
            .toArray();
